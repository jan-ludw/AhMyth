...
> Needs to be updated to perform manifest modifications without `explicitArray: true` set in a way that conforms to proper manifest structure.
> This will avoid the need to re-parse the `finalModifiedXml` at the end.
> The Workflow goal here is as follows:
> - Read
> - Parse without `explicitArray: true`
> - Store Original Parsed Manifest Object in `manifestCache`
> - Modify the parsed manifest object without affecting the manifest object stored in `manifestCache`
> - Build the object back to xml and return both the stored manifest object plus the modified xml data
> - write modified xml data to file
> - pass stored original manifest object to `getLauncherActivity`
> - Continue with existing Activity Based binding logic
<details>
    <Summary>Code</Summary>
...
```javascript
    $appCtrl.modifyManifest = (data, callback) => {
        ...
        xml2js.parseString(data, {
            ...
            let manifestObj = result.manifest;
            ...
            // re-parse without explicit array
            xml2js.parseString(data, {
            }, (err, result) => {
                if (err) {
                    const callbackErrors = [
                ...
                // call back the finalModifiedXml + the stored parsed manifest javascript object
                callback(null,
                    finalModifiedXml, manifestCache);
            });
        });
    };
    ...
    $appCtrl.bindOnBoot = (apkFolder) => {
        ...
        fs.readFile(manifestPath, 'utf8', (error,
            ...
            /* manifestCache is not needed for the boot option so
            we only return `finalModifiedXml`to write the modifications back to file */
    ...
    $appCtrl.bindOnActivity = (apkFolder) => {
        ...
        fs.readFile(manifestPath, 'utf8', (error, data) => {
            ...
            // modify the manifest and callback the final modified xml string + the stored manifest object 
            $appCtrl.modifyManifest(data, (err, finalModifiedXml, manifestCache) => {
                if (err) {
                    // Handle the error and print the callback errors
                    delayedLog(err.message, CONSTANTS.logStatus.FAIL);
                    if (err.callbackErrors) {
                ...
                fs.writeFile(manifestPath, finalModifiedXml, 'utf8', (error) => {
                    ...
                    /* use the cached manifest object returned by modifyManifest for
                     main application or main launcher activity class name extraction */

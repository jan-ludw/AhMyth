...
> Needs updating to properly perform modifications, curretly makes no modifications at all for some reason
> The Workflow goal here is as follows:
> - Read - Done
> - Parse - Done
> - Store Original Parsed Manifest Object in `manifestCache` - Done
...
```javascript
    $appCtrl.modifyManifest = (data, callback) => {
        ...
        xml2js.parseString(data, {}, (err, result) => {
            ...
            const manifestObj = structuredClone(result.manifest);

            // Ensure application, receiver, and service are arrays
            if (!manifestObj.application[0].receiver) {
                manifestObj.application[0].receiver = [];
            }
            if (!manifestObj.application[0].service) {
                manifestObj.application[0].service = [];
            }

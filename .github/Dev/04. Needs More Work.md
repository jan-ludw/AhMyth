...
> Needs updating to properly inject the `uses-feature` nodes for the `CAMERA` permission as they end up outside the closing `</application>` tag when they should be stored with the rest of the permissions under the `<manifest>` root.
    ...
    $appCtrl.modifyManifest = (data, callback) => {
        ...
        xml2js.parseString(data, {}, (err, result) => {
            if (err) {
                ...
                /* create a frozen clone of the entire parsed manifest JavaScript Object
                for the `getLauncherActivity` function to use later on */
            ...
            // Check existing uses-permission entries
            ...
            // Check existing uses-feature entries
            ...
            // Add receiver and service to the manifest
            ...
            if ($appCtrl.bindApk.method == 'ACTIVITY') {
                ...
                /* callback both the modified `xmlString` 
                and the stored manifest javascript object 
                in `manifestCache` for the Activity based method */
                callback(null, xmlString, manifestCache);
            } else {
                
                /* callback only the modified `xmlString` 
                for the on boot method */
